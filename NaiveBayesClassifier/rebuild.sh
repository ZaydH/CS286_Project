...
export JAR_NAME=naive_bayes.jar

rm -rf classes
mkdir classes
rm -f $JAR_NAME

javac -d classes NaiveBayesClassifier.java
jar -cvf $JAR_NAME -C classes/ .
javac -classpath $CLASSPATH:$JAR_NAME -d classes NaiveBayesMapper.java
javac -classpath $CLASSPATH:$JAR_NAME -d classes NaiveBayesReducer.java
jar -cvf $JAR_NAME -C classes/ .
